...
function AreaPanel({
  ...
  function getDeckContainingCard(cardId) {
    return Object.values(decks || {}).find((deck) =>
      (deck.cardIds || []).includes(cardId)
    );
  }

  function viewerCanSeeCardFace(card) {
    const containingDeck = getDeckContainingCard(card.id);

    if (containingDeck?.role === "hand") {
      return containingDeck.ownerId && containingDeck.ownerId === viewerId;
    }
