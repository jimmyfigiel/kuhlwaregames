...
export function scaleRectFromCenter({ rect, areaWidth, areaHeight, scale }) {
...
export function screenPointToAreaPoint({ screenX, screenY, areaWidth, areaHeight, scale }) {
  const centerX = areaWidth / 2;
  const centerY = areaHeight / 2;

  return {
    x: centerX + (screenX - centerX) / scale,
    y: centerY + (screenY - centerY) / scale,
  };
}
