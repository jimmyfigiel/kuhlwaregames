...
function normalizeSubmittedAction(playerSlot, action) {
  if (action && typeof action === "object" && action.type) {
    return action;
  }

  if (playerSlot && typeof playerSlot === "object" && playerSlot.type) {
    return playerSlot;
  }

  if (playerSlot && typeof playerSlot === "object" && playerSlot.action) {
    return playerSlot.action;
  }
...
export function submitAction(state, playerSlot, action) {
  ...
  const submittedAction = normalizeSubmittedAction(playerSlot, action);

  if (!submittedAction || !submittedAction.type) {
    return safeState;
  }

  switch (submittedAction.type) {
    case "EXECUTE_QUEUE": {
