...
function QueueManager({ gameState, submitAction }) {
  ...
  const runtime = gameState.commandRuntime || {
    status: "idle",
    activeCommand: null,
  };

  const currentCommand = runtime.activeCommand || commandQueue[0] || null;
  const pendingSteps = commandQueue.length;
  const hasActiveCommand = Boolean(runtime.activeCommand);
  ...
  function handleActiveCommandOk() {
    submitAction({
      type: "RESOLVE_ACTIVE_COMMAND",
    });
  }

  const buttonLabel = hasActiveCommand ? "Waiting" : "Execute";
  const isExecuteDisabled = hasActiveCommand || pendingSteps === 0;
    ...
    <section className="fp-queue-manager">
      <div className="fp-queue-header">
        ...
        <div className="fp-step-count">
          <span className="fp-step-number">{pendingSteps}</span>
          <span className="fp-step-label">
            {pendingSteps === 1 ? "queued step" : "queued steps"}
          </span>
        </div>
      </div>
      ...
      <div className="fp-current-command">
        {currentCommand ? (
          <>
            <div>
              <p>
                Status: <strong>{runtime.status || "idle"}</strong>
              </p>
              <p>
                Command type: <strong>{currentCommand.type}</strong>
              </p>
            </div>
      ...
      <div className="fp-queue-debug">
        Pending: {pendingSteps} · Active: {hasActiveCommand ? "yes" : "no"}
      </div>

      {runtime.activeCommand?.type === "popupMessage" && (
        <div className="fp-modal-backdrop">
          <div className="fp-modal-card" role="dialog" aria-modal="true">
            <h2>{runtime.activeCommand.title}</h2>
            <p>{runtime.activeCommand.message}</p>

            <button className="fp-primary-button" onClick={handleActiveCommandOk}>
              {runtime.activeCommand.buttonText || "OK"}
            </button>
          </div>
        </div>
      )}
...
function AccordionSection({ title, children, defaultOpen = false }) {
  return (
    <details className="fp-accordion-section" open={defaultOpen}>
      <summary className="fp-accordion-header">
        <span>{title}</span>
        <span className="fp-accordion-indicator">+</span>
      </summary>

      <div className="fp-accordion-body">{children}</div>
    </details>
  );
}
...
export default function FiveParsecsProcedureView({ gameState, submitAction }) {
  const safeGameState = useMemo(() => {
    return {
      commandRuntime: gameState?.commandRuntime || {
        status: "idle",
        activeCommand: null,
      },
