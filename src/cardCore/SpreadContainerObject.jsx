...
function isUpwardContainer(container) {
...
function getNestedItemLocalRect(container, item, index, effectiveSize = null) {
  ...
  if (isVerticalContainer(container)) {
    return {
      left: 0,
      top: isUpwardContainer(container)
        ? (effectiveSize?.height || container.height || height) - height - index * spacing
        : index * spacing,
      width,
      height,
    };
    ...
    return (
      ...
      >
        {previewItems.map((item, itemIndex) => {
          const localRect = getNestedItemLocalRect(column, item, itemIndex, effectiveSize);
